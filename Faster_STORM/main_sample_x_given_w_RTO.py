...
from scipy.sparse import vstack, dia_array
...
def sample_x_RTO(w):
    ...
    # standard lsmr
    L2 = dia_array((1/np.sqrt(w), 0), shape=(d, d))
    M = vstack( (L1TA, L2) ) #hstack( (ATL1, L2) ).T
    # t = time.time()
...
if __name__ == '__main__':
    ...
    sam = utils.load(PureWindowsPath(r'Faster_STORM\Runs', 'conf'+conf, sam_dir, 'sam'+sam_nr, 'par'))
    sam_w = utils.load(sam['sam_dir_w_samples'] / 'par')
    
    # general input
    in_gen = d2, sam_w['I'], sam_w['Ic'], delta
    ...
    for ii in range(sam_w['n_ch']):
        ...
        for jj in range(sam_w['N_po']//sam_w['N_save']):
            ...
            # get samples
            x_sam = np.zeros((d2, sam_w['N_save']))
            if sam['n_cores']>1:
    ...
    eval_samples.main(sam['sam_dir'] / 'samples_x', sam_w['n_ch'], sam_w['N_po']//sam_w['N_save'], flags, options = {'CI': 0.99})
